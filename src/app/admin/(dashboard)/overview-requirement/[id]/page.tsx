...
export default function EditOverviewRequirementPage() {
    ...
    const fetchAll = async () => {
      try {
        ...
        const [requirementRes, prodiRes] = await Promise.all([
          fetch("https://localhost:7013/api/OverviewRequirement", { headers }),
        ...
        if (requirementRes.ok) {
          const json = await requirementRes.json();
          const allItems: OverviewRequirementItem[] = Array.isArray(json) ? json : json.data ?? [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("https://localhost:7013/api/OverviewRequirement", {
        method: "PUT",
        headers: {
      ...
      router.push("/admin/overview-requirement");
    } catch {
  ...
  return (
    <div className="space-y-6">
      <Link
        href="/admin/overview-requirement"
        className="inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-800 transition-colors"
      >
        <ArrowLeft size={16} />
        Kembali ke Overview Requirement
      </Link>

      <div className="bg-white rounded-xl border border-gray-200 p-6 max-w-2xl">
        <h2 className="text-lg font-semibold text-gray-800 mb-6">Edit Overview Requirement</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              required
              rows={6}
              placeholder="Masukkan teks overview requirement..."
